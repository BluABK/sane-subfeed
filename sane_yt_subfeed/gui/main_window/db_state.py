...
class DbStateIcon(QLabel):
    ...
    def change_state(self, state):
        ...
        if state == DatabaseListener.DB_STATE_READ_WRITE:
            self.logger.db_info("Changing db state icon to read/write".format(state))
            ...
            self.logger.db_info("Changing db state icon to write".format(state))
            self.draw_write_icon(self.full_icon, self.write_dot, painter)
        elif state == DatabaseListener.DB_STATE_READ:
            self.logger.db_info("Changing db state icon to read".format(state))
            self.draw_read_icon(self.full_icon, self.read_dot, painter)
        else:
            self.logger.db_info("Changing db state icon to idle".format(state))
        self.setPixmap(self.full_icon)
